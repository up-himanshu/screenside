...
export class JuegoComponent implements OnInit, OnDestroy {
    ...
    this.playerService.currentMessage.subscribe(isOpen => {
      ...
      if (isOpen.length != 0) {
        if (this.vuelta < 1) {
          ...
          alert("Your shifts are over.");
          location.href = "http://127.0.0.1:4200/menujugar";  
  ...
  public Pantalla() {
    if (
      this.datos.user_id_one == this.currentUser.id &&
      this.NumeroPantalla == 0
    ) {
      this.NumeroPantalla = 1;
    ...
    if (
      this.datos.user_id_two == this.currentUser.id &&
      this.NumeroPantalla == 0
    ) {
      this.NumeroPantalla = 2;
  ...
  public creacionPiedra(e) {
    if (this.maxPiedra) {
      ...
      pie.setAttribute(
        "style",
        "z-index: 4; position: absolute; width: 25px; height: 25px; left: " +
          x +
          "px; top: " +
          posicion +
          "px"
  ...
  public checkCollisions(id) {
    ...
    if (
      choque_a.l <= choque_b.r &&
      choque_a.r >= choque_b.l &&
      choque_a.b >= choque_b.t &&
      choque_a.t <= choque_b.b
    ) {
